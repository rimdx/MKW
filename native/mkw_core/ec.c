...
mkw_ecc_point_t *
mkw_ecc_point_set(mkw_ecc_point_t *x, const mkw_ecc_point_t *n)
{
    memcpy(x, n, sizeof(*x));
    return x;
}
...
{
    mkw_ecc_point_t base2 = { 0 }; 
    mkw_ecc_point_t tmp = { 0 };
    mkw_bigint_set(&base2.x, &curve->g.x);
    mkw_bigint_set(&base2.y, &curve->g.y);
    ...
    for (int bit = 0; bit < MKW_BIGINT_BITS; bit++) {
        ...
        if (mkw_bigint_getbit(n, bit)) {
            ...
            mkw_ecc_point_add(result, curve,
                              mkw_ecc_point_set(&tmp, result),
                              &base2);
        }

        /* base2 *= 2 */
        mkw_ecc_point_set(&tmp, &base2);
        mkw_ecc_point_add(&base2, curve, &tmp, &tmp);
    }
}
